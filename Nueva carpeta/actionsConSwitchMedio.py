      ...
      self, dispatcher, tracker: Tracker, domain: Dict[Text, Any]
        ...
        if (Id != idSlot) and (fueChau == False): #guardo el perfil del que estaba antes
            ...
            if len(Juegos) > 0:
                ...
                juego = None
            guardar_perfil(Id,nombre,Categorias,JuegosGustan,JuegosNoGustan,juego,False)
        ############ cargar perfil
...
async def mi_funcion_asincrona(tracker, dispatcher):
    ...
    def run(
      self, dispatcher, tracker: Tracker, domain: Dict[Text, Any]
        ...
        if (IdActual != idSlot) and (fueChau == False):
            asyncio.run(mi_funcion_asincrona(tracker, dispatcher))
            return[]
        else:
      ...
      self, dispatcher, tracker: Tracker, domain: Dict[Text, Any]
        ...
        if (IdActual != idSlot) and (fueChau == False):
            ...
            if len(juegos) > 0:
                ...
                message = f"tpero si no te dije ningun juego"
            dispatcher.utter_message(text=message)
...
def guardar_perfil(id, nombre, Categorias, JuegosGustan, JuegosNoGustan, LastGame, Chau):
    # Guardar perfil
    texto = pd.read_csv('C:/Users/AGUSTIN/Documents/BotExploratoria/Perfiles/perfiles.csv', sep=';')
      ...
      self, dispatcher, tracker: Tracker, domain: Dict[Text, Any]
        ...
        if (IdActual != idSlot) and (fueChau == False):
            ...
            if len(Juegos) > 0:
                ...
                juego = None
            guardar_perfil(id,nombre,Categorias,JuegosGustan,JuegosNoGustan,juego,True)
